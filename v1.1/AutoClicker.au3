...
;           - Release CPS 1 to 20, ctrl to save mouse pos and space to launch / stop auto clicker if 'enabled' is checked
;
...
#AutoIt3Wrapper_Outfile_x64=AutoClicker_v1.1_x64.exe
...
$FormAutoClicker = GUICreate("Auto Clicker v1.1", 389, 115, 313, 204)
...
$LabelKeyAutoClicker = GUICtrlCreateLabel("Use ENTER to turn on/off auto clicker", 48, 80, 186, 17)
...
GUICtrlSetBkColor(-1, 0xE3E3E3)
GUICtrlSetState(-1, $GUI_DISABLE)
$LabelPos = GUICtrlCreateLabel("Use CTRL to set mouse position", 104, 40, 156, 17)
$ButtonAddCPS = GUICtrlCreateButton("+1", 48, 8, 27, 25)
$ButtonReduceCPS = GUICtrlCreateButton("-1", 0, 8, 27, 25)
$CheckboxEnable = GUICtrlCreateCheckbox("Enable", 296, 8, 57, 17)
GUICtrlSetState(-1, $GUI_CHECKED)
$LabelAuthor = GUICtrlCreateLabel("by LeGarri", 336, 96, 52, 17)
GUICtrlSetFont(-1, 4, 400, 0, "MS Sans Serif")
$CheckboxOnTop = GUICtrlCreateCheckbox("Always on top", 296, 24, 89, 17)
GUICtrlSetState(-1, $GUI_CHECKED)
GUISetState(@SW_SHOW)
...
While 1
	...
	If _IsPressed("0D") And Not $spaceBarPressed Then
		If GUICtrlread($CheckboxEnable) == $GUI_CHECKED Then
			If $click Then
	...
	If GUICtrlRead($CheckboxOnTop) == $GUI_CHECKED Then
		WinSetOnTop($FormAutoClicker, "", 1)
	Else
		WinSetOnTop($FormAutoClicker, "", 0)
	EndIf
